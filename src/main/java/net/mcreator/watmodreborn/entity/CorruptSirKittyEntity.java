...
public class CorruptSirKittyEntity extends TamableAnimal {
	public CorruptSirKittyEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(WatModRebornModEntities.CORRUPT_SIR_KITTY.get(), world);
	}

	public CorruptSirKittyEntity(EntityType<CorruptSirKittyEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(1f);
		xpReward = 2;
		setNoAi(false);
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		CorruptSirKittyEntity retval = WatModRebornModEntities.CORRUPT_SIR_KITTY.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(WatModRebornModEntities.CORRUPT_SIR_KITTY.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getDifficulty() != Difficulty.PEACEFUL && Monster.isDarkEnoughToSpawn(world, pos, random) && Mob.checkMobSpawnRules(entityType, world, reason, pos, random)));
		DungeonHooks.addDungeonMob(WatModRebornModEntities.CORRUPT_SIR_KITTY.get(), 180);
	}
