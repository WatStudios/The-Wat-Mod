...
import net.mcreator.watmodreborn.procedures.StormyEntityDiesProcedure;
...
public class StormyEntity extends TamableAnimal {
	public StormyEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(WatModRebornModEntities.STORMY.get(), world);
	}

	public StormyEntity(EntityType<StormyEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(1f);
		xpReward = 69;
		setNoAi(false);
	...
	public void die(DamageSource source) {
		super.die(source);
		StormyEntityDiesProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	}
			...
			if (sourceentity instanceof ServerPlayer serverPlayer) {
				NetworkHooks.openScreen(serverPlayer, new MenuProvider() {
					...
					public Component getDisplayName() {
						return Component.literal("Stormy");
					}
					...
					public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
						...
						packetBuffer.writeVarInt(StormyEntity.this.getId());
						return new StormyGUIMenu(id, inventory, packetBuffer);
					}
				}, buf -> {
	...
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		StormyEntity retval = WatModRebornModEntities.STORMY.get().create(serverWorld);
	...
	public static void init() {
		SpawnPlacements.register(WatModRebornModEntities.STORMY.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES,
				(entityType, world, reason, pos, random) -> (world.getBlockState(pos.below()).is(BlockTags.ANIMALS_SPAWNABLE_ON) && world.getRawBrightness(pos, 0) > 8));
		DungeonHooks.addDungeonMob(WatModRebornModEntities.STORMY.get(), 180);
	}
